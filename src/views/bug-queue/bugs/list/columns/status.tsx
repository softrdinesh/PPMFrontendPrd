...
interface InsertDynamicValuePayload {
  ...
  // FIX: BugID/GroupID are the fields actually used at the call-site (see
  // handleStatusChange below). They were missing from this interface, which
  // caused TS2353 ("'BugID' does not exist in type ...") and TS2552 on the
  // console.error object further down. Added as optional so both usages
  // type-check without altering any runtime behavior.
  BugID?: number;
  GroupID?: number;
...
interface UpdateStatusPayload {
  StatusID: number;
  TaskID: number;
  // FIX: LoginuserID is not always available (user?.id can be undefined at
  // the call-site), so this is made optional rather than a required number.
  LoginuserID?: number;
  GroupID: number;
  Statusname: string;
  Colorcode: string;
...
interface CreateTaskStatusPayload {
  Statusname: string;
  TaskID: number;
  // FIX: same reasoning as UpdateStatusPayload.LoginuserID above — user?.id
  // can be undefined, so this can no longer be a strictly required number.
  LoginuserID?: number;
  GroupID: number;
  Colorcode: string;
}
...
interface SprintTaskUpdatePayload {
  ...
  ActualSP?: number;
  isunplan?: boolean;
  // FIX: statusIDToSend can be explicitly `null` (to clear the status), so
  // the type must allow null in addition to number/undefined.
...
const insertDynamicValue = async (payload: InsertDynamicValuePayload): Promise<InsertDynamicValueResponse> => {
  try {
    ...
    const BugID = ((payload as any)?.BugID);
      ...
      <Box display={'flex'} alignItems={'stretch'} gap={2}>
        <Box
          ...
          onClick={async (event) => {
            ...
            // FIX: use ?? instead of || so a currentStatusId of 0 is respected,
            // and compare as numbers (Number(...) !== Number(...)) instead of
            // loose `!=` so mixed string/number ids from different sources
            // (row data vs dynamic value) don't cause the click to be skipped.
            const currentStatusId = row?.StatusID ??
                                   dynamicValue?.Status?.StatusID ??
                                   dynamicValue?.statusID

            if (Number(currentStatusId) !== Number(item?.StatusID)) {
              await handleStatusChange();
              // NOTE: removed the extra refetch() call here — handleStatusChange
              // (via onStatusChangeFromParent / handleUpdateStatus in BugGroupTable,
              // or the dynamic/non-dynamic branches above) already triggers its
              // own refetch, so calling it again here was redundant.
            }
...
const rolename = parsedData?.rolename;
    ...
    select: (data) => {
      ...
      if (Array.isArray(data)) {
        ...
        return data.map((item: StatusLookupItem) => ({
          ...
          IsDefault: false,
          TaskgroupID: null
        })) as unknown as ProjectStatusList[];
      }
      return [];
    }
  })
  ...
  const statusName = useMemo(() => {
    ...
    const flatName = (row as any)?.StatusName || (row as any)?.statusname
    if (flatName) return flatName

    // FIX: last-resort lookup by StatusID against known status lists
    ...
    if (rowStatusID !== undefined && rowStatusID !== null) {
      const found = [...(statusList || []), ...(dynamicStatus || [])].find(
        (s: any) => Number(s?.StatusID) === Number(rowStatusID)
      )
      if (found?.Statusname) return found.Statusname
    }

    return null
  }, [columnData, dynamicValue, row, statusList, dynamicStatus])
  ...
  const handleEdit = (item?: ProjectStatusList) => {
    ...
    setIsEdit(item?.StatusID?.toString() ?? null)
  ...
  const handleDeleteConfirm = async () => {
    ...
    try {
      ...
      const response = await deleteStatus(
        ...
        row,
        selected?.WorkspaceID || workspaceID || 0
      );
      // if (response?.status) {
        // toast.success('Status Deleted Successfully');
        refetchStatusList();
  ...
  const onSubmit = async (data: FormValidateType) => {
    if (isEdit) {
      ...
      const response = await updateStatus({
        ...
        LoginuserID: user?.id, // FIX: required by UpdateStatusPayload; was missing entirely
