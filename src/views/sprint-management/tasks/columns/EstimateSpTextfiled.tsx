...
export const EstimateSpTextfiled = ({ table, getValue, index, id, canEdit = true }: ColumnTextFieldProps) => {
    ...
    <Box sx={{ position: 'relative', width: '100%' }}>
      ...
      disabled={rolename =='Viewer'}
        ...
        sx={{
          ...
          '& .MuiInputBase-root:hover::before': { borderBottom: 0 },// Using theme for better dark/light mode support
...
export const defaultColumn: any = (canEdit: boolean) => ({
  cell: ({ getValue, row: { index }, column: { id }, table }: DefaultColumnType) => {
    return <EstimateSpTextfiled getValue={getValue} index={index} id={id} table={table} canEdit={canEdit} />
  }
})
