...
const DynamicStatus = ({ columnData = null, rowData = null, dynamicValue = null, refetch, isSubTask }) => {
  ...
  const { data: dynamicStatus, refetch: refetchStatusList } = useQuery({
    queryKey: ['dynamic-status', rowData?.TaskGroupID],
    queryFn: () => fetchProjectStatusList({ taskGroupID: rowData?.TaskGroupID })
  })
    ...
    <Box display={'flex'} alignItems={'center'} height={'100%'}>
      <Tooltip title={dynamicValue?.Status?.Statusname}>
        <Box
          ...
          display={'flex'}
          alignItems={'center'}
          color={getContrastingTextColor(generateBGColor(dynamicValue?.Status?.Colorcode))}
          px={2}
